...
@section('content')
       ...
       <div class="modal-body text-left">
                ...
                Form::open(
                array(
                'route' => 'assign_user_application',
                'name' => 'assignUserapplication',
                'autocomplete' => 'off', 
                'id' => 'assignUserApplication',
                'target' => '_top',
                'method'=> 'POST'
                )
                )
                ...
                @foreach ($allCollectedData['toAssignedData']->selected_application as $value)
                    <input type="hidden" name="selected_application[]" value="{{$value}}">
                @endforeach
...
<script>
    ...
    $( "#assignUserApplication" ).submit(function( event ) {
        event.preventDefault();
        var selected_user = $('#role_user').val().length;
        $('#user_error').text('');
        if(selected_user != 0){
            
            document.getElementById("assignUserApplication").submit();
        }else{
