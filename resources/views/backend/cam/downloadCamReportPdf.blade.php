                        ...
                        <table id="invoice_history" class="table   no-footer overview-table " role="grid" aria-describedby="invoice_history_info" cellpadding="0" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr role="row" class="odd">
                                    <td class="">PTPQ</td>
                                    <td class="">
                                        @php $i = 1; if(!empty($leaseOfferData->offerPtpq)){ $total = count($leaseOfferData->offerPtpq); @endphp @foreach($leaseOfferData->offerPtpq as $key => $arr) @if ($i > 1 && $i
                                        < $total) , @elseif ($i> 1 && $i == $total) and @endif Rs. {{$arr->ptpq_rate}} for {{floor($arr->ptpq_from)}}- {{floor($arr->ptpq_to)}} {{$arrStaticData['rentalFrequencyForPTPQ'][$leaseOfferData->rental_frequency]}} @php $i++; @endphp @endforeach @php } @endphp

                                    </td>
                                </tr>
                                <tr role="row" class="odd">
                                    <td class="" valign="top">XIRR</td>
                                    <td class="" valign="top">Ruby Sheet : {{isset($leaseOfferData->ruby_sheet_xirr) ? $leaseOfferData->ruby_sheet_xirr : ''}}%
                                        <br>Cash Flow : {{isset($leaseOfferData->cash_flow_xirr) ? $leaseOfferData->cash_flow_xirr : ''}}%
                                    </td>
                                </tr>
                                ...
                                <tr role="row">
                                    <td align="center" rowspan="">
                                        <table class="table  no-footer overview-table " role="grid" aria-describedby="invoice_history_info" cellpadding="0" cellspacing="0">
                                            @php $i=0; @endphp @while(!empty($arrCM[$i]))
                                            <tr>
                        ...
                        <table id="invoice_history" class="table   no-footer overview-table " role="grid" aria-describedby="invoice_history_info" cellpadding="0" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <p class="m-0">
                                            - Minimum 3 years of vintage in relevant business
                                            <br> - Parent or group company with requisite vintage
                                            <br> - Key promoter with 5 years of relevant vintage
                                        </p>
                                    </td>
                                    <td>No</td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {{ isset($arrHygieneData->kyc_risk_check) && $arrHygieneData->kyc_risk_check == 'High' ? 'High' : '' }} {{ isset($arrHygieneData->kyc_risk_check) && $arrHygieneData->kyc_risk_check == 'Med' ? 'Medium' : ''}} {{ isset($arrHygieneData->kyc_risk_check) && $arrHygieneData->kyc_risk_check == 'Low' ? 'Low' : '' }} {{ isset($arrHygieneData->kyc_risk_check) && $arrHygieneData->kyc_risk_check == 'No' ? 'No' : '' }}
                                    </td>
                                    <td>
                                        {{ isset($arrHygieneData->kyc_risk_check) && $arrHygieneData->kyc_risk_check == 'High' ? 'Highf' : '' }} {{ isset($arrHygieneData->kyc_risk_check) && $arrHygieneData->kyc_risk_check == 'Med' ? 'Medium' : ''}} {{ isset($arrHygieneData->kyc_risk_check) && $arrHygieneData->kyc_risk_check == 'Low' ? 'Low' : '' }} {{ isset($arrHygieneData->kyc_risk_check) && $arrHygieneData->kyc_risk_check == 'No' ? 'No' : '' }}
                                    </td>
                        ...
                        <table id="invoice_history" class="table   no-footer overview-table " role="grid" aria-describedby="invoice_history_info" cellpadding="0" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>- IT assets and telecommunications max 70%
                                        <br>- Plant and machinery max 50%
                                        <br>- Furniture and fit outs max 30%
                ...
                <div class="data mt-4">
                    ...
                    <div class="pl-4 pr-4 pb-4 pt-2">
                        ...
                        <table class="table table-bordered overview-table">
                            ...
                            <tbody>
                                @foreach($arrOwnerData as $key => $arrData) @if ($arrData->is_promoter)
                                <tr>
                                    <td>{{$arrData->first_name}}</td>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="vertical-align:top; padding:0px !important; border-right:none;">
                                        <table class="table-border-none" width="100%">
                                            <tbody>
                                                @php $yearly_fin_data = getTotalFinanceData($fin_data) @endphp @foreach($FinanceColumns as $key => $cols)
                                                <tr>
                                                    <td height="46" align="right">
                                                        <input type="text" class="form-control form-control-sm" disabled value="{{sprintf('%.2f', $yearly_fin_data[$key] ?? '')}}">
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
