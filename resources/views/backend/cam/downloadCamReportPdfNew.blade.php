...
<html lang="en">
    ...
    border-top: #e2e2e2 solid 1px;
		...
		table td.blank{ background-color:#cccccc !important;
			 -webkit-print-color-adjust: exact;}
             p{margin:0px;}
             .data {border: 1px solid #e9ecef; margin-bottom: 25px;}
             .sub-title.bg {
    background: #efefef;
    padding: 10px 15px;
    width: 100%;
...
.pl-4.pr-4.pb-4.pt-2{padding:15px;clear: both;}
    </style>

    </head>
    <body>

	<table width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:14px;  border:none;">
	<tr><td align="center"><b>CAM Report For {{isset($arrBizData->biz_entity_name) ? $arrBizData->biz_entity_name : ''}}</b></td></tr>
                         ...
                         <table class="table-border-none" width="100%" cellpadding="0" cellspacing="0">
                            <tbody>
                              ...
                              @foreach($FinanceColumns as $key => $cols)
                                <tr>
                                  <td align="right"><input type="text" class="form-control form-control-sm" disabled value="{{sprintf('%.2f', $yearly_fin_data[$key] ?? '')}}"></td>
                               </tr>
                               @endforeach
                            </tbody>
                         </table>
