...
<div class="data mt-4">
    ...
    <div class="col-md-12 mt-4" id="security-doc-block">
        @if(!empty($arrAppSecurityDoc))
            ...
            {{-- @if ($data['curr_status_id'] = 21) --}}
            {{-- @php
                print_r($arrAppSecurityDoc); die;
            @endphp --}}
            ...
            @php
              ...
              @endphp
              {{-- @php
                  echo($arr['is_non_editable'] == 1); die;
              @endphp --}}
              @if($arr['is_non_editable'] == 1 && $arr['status'] == 2)
            <div class="row p-2 mt-1 toRemoveDiv1 {{($loop->first)? '': 'mt10'}}" style="background-color: #e9e7e7;">
                ...
                <div class="col-md-2 mt-1">
                    ...
                    <div class="relative">
                        <select class="form-control doc_type" name="doc_type[]" id="update_doc_type_{{ $key }}" disabled>
                            ...
                            <option value="1" {{(isset($arr['doc_type']) && $arr['doc_type'] == '1') ? 'selected': '' }}>Pre</option>
                            <option value="2" {{(isset($arr['doc_type']) && $arr['doc_type'] == '2') ? 'selected': '' }}>Post</option>
                        </select>
                    </div>
                </div>
                <div class="col-md-2 mt-1">
               ...
               <div class="col-md-2 mt-1">
                    ...
                    <div class="relative">
                       <input type="text" name="document_number[]" class="form-control document_number" value="{{$arr['document_number'] ?? ''}}" placeholder="Document Number" autocomplete="off" id="update_document_number_{{ $key }}" disabled/>
                    </div>
               </div>
               @endif
               <div class="col-md-2 mt-1">
                    ...
                    <div class="relative">
                            <input type="text" name="due_date[]" maxlength="20" class="form-control sc-doc-date due_date" value ="{{(isset($arr['due_date']) && $arr['due_date']) ? \Carbon\Carbon::createFromFormat('Y-m-d', $arr['due_date'])->format('d/m/Y'): '' }}" placeholder="Original Due Date" autocomplete="off" id="update_due_date_{{ $key }}" readonly="readonly" disabled/>
                    </div>
               </div>
               @if($route_name=="security_deposit")  
               <div class="col-md-2 mt-1">
                   ...
                   <div class="relative">
                       <select class="form-control completed" name="completed[]" id="update_completed_{{ $key }}" disabled>
                           <option value="">Select</option>
              ...
              <div class="col-md-2 mt-1">
                       ...
                       <div class="relative">
                           <select class="form-control exception_received" name="exception_received[]" onchange="displayExceptionFields(this.value,{{ $key }});" id="update_exception_received_{{ $key }}" data-previous="{{ $key }}" disabled>
                               <option value="">Select</option>
               ...
               <div class="col-md-2 mt-1 exceptionFields_{{ $key }}" {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'no' || $arr['exception_received'] == null) ? 'style="display: none;"': '' !!}>
                       ...
                       <div class="relative">
                       <input type="text" name="exception_received_from[]" class="form-control exception_received_from {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'yes') ? 'required': '' !!}" value="{{$arr['exception_received_from'] ?? ''}}" placeholder="Exception Received From" autocomplete="off" id="update_exception_received_from_{{ $key }}" {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'no') ? 'style="visibility: none;height: 0;"': '' !!} disabled/>
                       </div>
               </div>
               <div class="col-md-2 mt-1 exceptionFields_{{ $key }}" {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'no' || $arr['exception_received'] == null) ? 'style="display: none;"': '' !!}>
                       <label for="txtPassword"><b>Exception Received Date</b></label>
                       <div class="relative">
                       <input type="text" name="exception_received_date[]" class="form-control sc-doc-date-r exception_received_date {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'yes') ? 'required': '' !!}" value ="{{(isset($arr['exception_received_date']) && $arr['exception_received_date']) ? \Carbon\Carbon::createFromFormat('Y-m-d', $arr['exception_received_date'])->format('d/m/Y'): '' }}" placeholder="Exception Received Date" autocomplete="off" id="update_exception_received_date_{{ $key }}" {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'no') ? 'style="visibility: hidden;height: 0;"': '' !!} readonly="readonly" disabled/>
                       </div>
               </div>
               <div class="col-md-2 mt-1 exceptionFields_{{ $key }}" {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'no' || $arr['exception_received'] == null) ? 'style="display: none;"': '' !!}>
                   <label for="txtPassword"><b>Exception Remark</b></label>
                   <div class="relative">
                   <input type="text" name="exception_remark[]" class="form-control exception_remark {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'yes') ? 'required': '' !!}" value="{{$arr['exception_remark'] ?? ''}}" placeholder="Exception Remark" autocomplete="off" id="update_exception_remark_{{ $key }}" {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'no') ? 'style="visibility: hidden;height: 0;"': '' !!}disabled />
                   </div>
               </div>
               <div class="col-md-2 mt-1 exceptionFields_{{ $key }}" {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'no' || $arr['exception_received'] == null) ? 'style="display: none;"': '' !!}>
                    <label for="txtPassword"><b>Extended Due Date</b></label>
                    <div class="relative">
                    <input type="text" name="extended_due_date[]" class="form-control sc-doc-date-r extended_due_date {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'yes') ? 'required': '' !!}" value ="{{(isset($arr['extended_due_date']) && $arr['extended_due_date']) ? \Carbon\Carbon::createFromFormat('Y-m-d', $arr['extended_due_date'])->format('d/m/Y'): '' }}" placeholder="Extended Due Date" autocomplete="off" id="update_exception_received_date_{{ $key }}" {!!(isset($arr['exception_received']) && $arr['exception_received'] == 'no') ? 'style="visibility: hidden;height: 0;"': '' !!} readonly="readonly" disabled/>
                    </div>
                </div>
               <div class="col-md-2 mt-1">
                   <label for="txtPassword"><b>Maturity Date</b></label>
                   <div class="relative">
                   <input type="text" name="maturity_date[]" class="form-control sc-doc-date maturity_date" value="{{ $arr['maturity_date']!=null?\Carbon\Carbon::createFromFormat('Y-m-d', $arr['maturity_date'])->format('d/m/Y'):'' ?? ''}}" placeholder="Maturity Date" autocomplete="off" id="update_maturity_date_{{ $key }}" readonly="readonly" disabled/>
                   </div>
               </div>
               <div class="col-md-2 mt-1">
                   <label for="txtPassword"><b>Renewal Reminder Days</b></label>
                   <div class="relative">
                   <input type="text" name="renewal_reminder_days[]" class="form-control digits renewal_reminder_days" value="{{$arr['renewal_reminder_days'] ?? ''}}" placeholder="Renewal Reminder Days" autocomplete="off" id="update_renewal_reminder_days_{{ $key }}" disabled/>
                   </div>
               </div>
               <div class="col-md-2 mt-1 INR">
                   ...
                   <input type="text" name="amount_expected[]" class="form-control number float_format amount_expected" value="{{$arr['amount_expected'] ?? ''}}" placeholder="Amount Expected" autocomplete="off" id="update_amount_expected_{{ $key }}" disabled/>
                   </div>
               </div>
               <div class="col-md-2 mt-1 INR">
                   ...
                   <div class="relative">
                   <a href="javascript:void(0);" class="verify-owner-no" ><i class="fa fa-inr" aria-hidden="true"></i></a>
                   <input type="text" name="document_amount[]" class="form-control number float_format document_amount" value="{{$arr['document_amount'] ?? ''}}" placeholder="Document Amount" autocomplete="off" id="update_document_amount_{{ $key }}" disabled/>
                   </div>
               </div>
                   ...
                   <div class="relative">
                    <div class="d-flex">
                       <div class="custom-file upload-btn-cls mb-3">
                           <input type="file" class="custom-file-input getFileName doc_file_sec {!!(isset($arr['is_upload']) && $arr['is_upload'] == 0) ? 'required': '' !!}" name="doc_file_sec[]" id="update_doc_file_{{ $key }}" disabled>
