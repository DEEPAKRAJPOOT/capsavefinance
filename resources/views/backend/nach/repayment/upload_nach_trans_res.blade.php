...
@section('content')
<form id="documentForm" style="width: 100%" method="POST" action="{{ Route('import_nach_trans_response') }}" enctype="multipart/form-data" target="_top">       
    @csrf
...
@if($operation_status == config('common.YES'))
    ...
    try {
        ...
        p.jQuery('#importNachTransResponse').modal('hide');
        p.location.reload();
    } catch (e) {
