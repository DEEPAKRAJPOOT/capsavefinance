...
<div class="modal-body text-left">
    <form id="locationytpeForm" name="locationytpeForm" method="POST" action="{{route('edit_location_type')}}"
        target="_top">
        @csrf
        <input type="hidden" name="location_id" id="location_id" value="{{ $location_data->location_id }}">
        <div class="row">
            <div class="form-group col-md-12">
                <label for="name">Location Type</label>
                <input type="text" class="form-control" id="name" name="name"
                    value="{{ isset($location_data) ? $location_data->name : "old('name')" }}"
                    placeholder="Enter Location Type" maxlength="50">
            </div>
        </div>
        <div class="row">
            <div class="form-group col-md-12">
                ...
                <input type="text" class="form-control" id="location_code" name="location_code"
                    value="{{ isset($location_data) ? $location_data->location_code : "old('location_code')" }}"
        ...
        <div class="row">
            <div class="form-group col-md-6">
                ...
                <select class="form-control" name="is_active" id="is_active">
                    ...
                    <option value="1"
                        value="{{ isset($location_data) ? $location_data->is_active : "old('location_code')" }}"
                        {{ isset($location_data) && $location_data->is_active == 1 ? 'selected' : '' }}>Active</option>
                    <option value="2"
                        value="{{ isset($location_data) ? $location_data->is_active : "old('location_code')" }}"
                        {{ isset($location_data) && $location_data->is_active == 2 ? 'selected' : '' }}>In-Active
                    </option>
                </select>
            </div>
        </div>
