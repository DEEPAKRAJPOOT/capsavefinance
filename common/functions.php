...
function getInvestmentPaymentFrequency($id){
	 $productType = ['1'=>'Upfront', 'Monthly', 'Rear Ended', 'Pre Offer'];
...
function getTotalFinanceData($fullArray, $prevFullArray = []){
	...
	$response['Depreciation'] = $fullArray['ProfitAndLoss']['Depreciation'];
	...
	$Prev_TotalRepaymentsDueWithin1Year = empty($PrevLiabilities) ? 0 :($PrevLiabilities['InstallmentsOfTermLoansDebenturesDpgsEtcDueWithin1Year'] +  $PrevLiabilities['DepositsDueForRepaymentDueWithin1Year'] +  $PrevLiabilities['PreferenceSharesRedeemableWithin1Year']);
	$netBlock = $curr_netblock + $prev_netblock;
	$a = ($netBlock/2);
	$response['DeprecationAverageNetFixedAssetsPer'] = ($a == 0) ? 0 : (($response['Depreciation'] / $a)  * 100);
	...
	$response['NetProfit'] = $response['TotalOperatingIncome']-($AddOpeningStockInProcessRawMaterials+$OtherSpares+$PowerFuel+$DirectLabour+$OtherManufacturingExpenses+$response['Depreciation']+$RepairsMaintenance+$CostOfTradingGoods+0+$AddOpeningStockInProcess-$DeductClosingStockInProcess+$AddOpeningStockOfFinishedGoods-$DeductClosingStockOfFinishedGoods+$SellingGeneralAdmExpenses)-$response['Interest']+$response['TotalNonOperatingIncome']+0-($LossOnSaleOfInvestments+$LossOnSaleOfFa+$DerivativeLossesBooked+$NetLossOnForeignCurrencyTranslationAndTransactionsLossDueToFire + $PreliExpOneTimeExpensesWrittenOff+$MiscExpWrittenOff+$ProvForDoubDebtsDimInTheValOfInv+$WealthTax)-$TaxPaid-($ProvisionForTaxesCurrentPeriod+$ProvisionForTaxesDefferedTaxes);
	$response['CashProfit'] = ((($response['TotalOperatingIncome']-(((($AddOpeningStockInProcessRawMaterials+$OtherSpares +$PowerFuel+$DirectLabour+$OtherManufacturingExpenses+$response['Depreciation']+$RepairsMaintenance+$CostOfTradingGoods)+$AddOpeningStockInProcess-$DeductClosingStockInProcess)+$AddOpeningStockOfFinishedGoods- $DeductClosingStockOfFinishedGoods)+$SellingGeneralAdmExpenses))-($InterestPaymentToBanksSum + $InterestPaymentToFIsSum + $BankCharges))+($InterestOnDepositsDividendReceived+ $ForexGains+$NonOperatingIncomeFromSubsidiaries+$TaxRefund+$MiscIncome+$ProfitOnSaleOfAssetsInvestments+$OtherIncome + $ProvisionsExpensesWrittenBack -($LossOnSaleOfInvestments+$LossOnSaleOfFa+$DerivativeLossesBooked+$NetLossOnForeignCurrencyTranslationAndTransactionsLossDueToFire+$PreliExpOneTimeExpensesWrittenOff+ $MiscExpWrittenOff + $ProvForDoubDebtsDimInTheValOfInv + $WealthTax))- $TaxPaid -($ProvisionForTaxesCurrentPeriod + $ProvisionForTaxesDefferedTaxes))+$response['Depreciation'] - $ProvisionsExpensesWrittenBack + $PreliExpOneTimeExpensesWrittenOff + $MiscExpWrittenOff + $ProvForDoubDebtsDimInTheValOfInv + $WealthTax;
	...
	$response['TolTnw'] = $response['TangibleNetWorth'] == 0 ? 0 :(CalculateTotalOutsideLiabilities($Liabilities) / $response['TangibleNetWorth']);
	...
	$response['TolAdjTnwAtnw'] = $AdjustedTangibleNetWorth == 0 ? 0 : ((CalculateTotalOutsideLiabilities($Liabilities) - $BorrowingsFromSubsidiariesAffiliatesQuasiEquity) / $AdjustedTangibleNetWorth);
	...
	$response['TotalOutsideLiabilities'] = CalculateTotalOutsideLiabilities($Liabilities);
...
function numberTowords(float $amount)
{
   ...
   $here_digits = array('', 'Hundred','Thousand','Lakh', 'Crore','Million','Billion');
