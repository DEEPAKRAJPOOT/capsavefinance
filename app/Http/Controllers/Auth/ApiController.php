  ...
  public function tally_entry(){
    ...
    if (!$txnsData->isEmpty() || !$paymentData->isEmpty()) {
        ...
        foreach ($txnsData as $key => $txn) {
            ...
            if (in_array($txn->trans_type, [config('lms.TRANS_TYPE.TDS'), config('lms.TRANS_TYPE.REFUND'), config('lms.TRANS_TYPE.NON_FACTORED_AMT'), config('lms.TRANS_TYPE.WAVED_OFF'),  config('lms.TRANS_TYPE.MARGIN')]) && $txn->entry_type == 0) {
               continue;
            }
