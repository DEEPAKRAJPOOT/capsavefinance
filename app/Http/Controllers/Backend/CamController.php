...
class CamController extends Controller
{
  ...
  public function saveReviewerSummary(Request $request)
  {
    try {
      ...
      $arrAppSecurityQuerry = AppSecurityDoc::where(['app_id' => $arrData['app_id'],'biz_id'=>$arrData['biz_id'], 'is_active' => 1,'is_non_editable'=>1,'status'=>2])->first();
      if(empty($arrAppSecurityQuerry)){
        if (isset($arrData['security_doc_id']) && !empty($arrData['security_doc_id']) && isset($arrData['doc_type']) && !empty($arrData['doc_type'])) {
          $dataCheck = array_filter($arrData['security_doc_id']);
          $dataCheck1 = array_filter($arrData['doc_type']);
  ...
  public function analyse_bank(Request $request)
  {
    ...
    if ($response['status'] == 'success') {
      return response()->json([
        'message' => 'Bank Statement analysed successfully.', 'status' => 1,
        'value' => $response
      ]);
    } else {
  ...
  public function analyse_finance(Request $request)
  {
    ...
    if ($response['status'] == 'success') {
      return response()->json([
        'message' => 'Financial Statement analysed successfully.', 'status' => 1,
        'value' => $response
      ]);
    } else {
  ...
  public function getBankReport(Request $request)
  {
    ...
    if ($final_res['status'] == 'success') {
      return response()->json([
        'message' => 'Banking Statement analysed successfully.', 'status' => 1,
        'value' => $final_res
      ]);
    } else {
      return response()->json(['message' => $final_res['message'] ?? 'Something went wrong', 'status' => 0, 'value' => ['file_url' => '']]);
  ...
  public function gstin(Request $request, FinanceModel $fin)
  {
    ...
    return view('backend.cam.gstin', [
      'gstdocs' => $gstdocs, 'appId' => $appId, 'gst_no' => $gst_no, 'all_gst_details' => $all_gst_details, 'currenttop3Cus' => $currenttop3Cus,
  ...
  public function saveSecurityDeposit(Request $request)
  {
    ...
    try {
        ...
        if (isset($arrCamData['security_doc_id']) && !empty($arrCamData['security_doc_id']) && isset($arrCamData['doc_type']) && !empty($arrCamData['doc_type'])) {
          ...
          $dataCheck7 = count($arrCamData['renewal_reminder_days']);
        ...
        $whereActivi['activity_code'] = 'security_deposit_save';
        $activity = $this->mstRepo->getActivity($whereActivi);
        if (!empty($activity)) {
          $activity_type_id = isset($activity[0]) ? $activity[0]->id : 0;
