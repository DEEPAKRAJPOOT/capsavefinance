...
class DataRenderer implements DataProviderInterface
{
    ...
    public function getAppList(Request $request, $app)
    {
        return DataTables::of($app)
                ->rawColumns(['app_id','assignee', 'assigned_by', 'status', 'action','contact','name', 'app_code'])
                ->addColumn(
                    'app_code',
                    function ($app) {
                        ...
                        if (!empty($parent_app_id)) {
                            ...
                            $ret .= "<br><small>Parent:</small><br><a href='" . route('company_details', ['biz_id' => $aData->biz_id, 'app_id' => $parent_app_id]) . "' rel='tooltip'>" . 'CAPAI'. $parent_app_id . "</a>";
                        } 
