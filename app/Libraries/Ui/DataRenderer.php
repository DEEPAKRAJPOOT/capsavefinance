...
class DataRenderer implements DataProviderInterface {
                            ...
                            if ($request->get('is_assign') != '') {
                                if ($request->has('is_assign')) {
                                    $query->where(function ($query) use ($request) {
                                        ...
                                        $query->where('users.is_assigned', 'like', "%$by_status%");
                                    });
                                }
                            }
                        })
    ...
    public function getAppList(Request $request, $app) {
        return DataTables::of($app)
                        ...
                        ->addColumn(
                                'action', function ($app) use ($request) {
                            $act = '';
    ...
    public function getFiRcuAppList(Request $request, $app) {
        return DataTables::of($app)
                        ...
                        ->addColumn(
                                'user_name', function ($app) {
                            return $app->f_name . ' ' . $app->m_name . ' ' . $app->l_name;
                        })
                        ->addColumn(
                                'user_email', function ($app) {
                            return $app->email;
                        })
                        ->addColumn(
                                'user_phone', function ($app) {
                        ...
                        ->addColumn(
                                'applied_loan_amount', function ($app) {
                            return $app->loan_amt ? number_format($app->loan_amt) : '';
                        })
                        ->addColumn(
                                'created_at', function ($app) {
    ...
    public function getBackendInvoiceList(Request $request, $invoice) {
        return DataTables::of($invoice)
                        ...
                        ->addColumn(
                                'invoice_due_date', function ($invoice) {
                        ...
                        ->addColumn(
                                'invoice_amount', function ($invoice) {
                        ...
                        ->addColumn(
                                'view_upload_invoice', function ($invoice) {
    ...
    public function getBackendInvoiceListDisbursedQue(Request $request, $invoice) {
        ...
        return DataTables::of($invoice)
                        ...
                        ->addColumn(
                                'anchor_id', function ($invoice) use ($request) {
                            if ($request->front) {
                                return '<a href="' . route("frontend_view_invoice_details", ["invoice_id" => $invoice->invoice_id]) . '">' . $invoice->invoice_no . '</a>';
                            } else {
                                return '<a href="' . route("view_invoice_details", ["invoice_id" => $invoice->invoice_id]) . '">' . $invoice->invoice_no . '</a>';
    ...
    public function getBackendInvoiceListFailedDisbursed(Request $request, $invoice) {
        ...
        return DataTables::of($invoice)
                        ...
                        ->addColumn(
                                'action', function ($invoice) use ($request) {
    ...
    public function getAllManualTransaction(Request $request, $trans) {
        ...
        return DataTables::of($trans)
                        ...
                        ->addIndexColumn()
                        ->addColumn(
                                'customer_id', function ($trans) {
                            return $trans->disburse ? $trans->disburse->customer_id : '';
                        })
                        ->addColumn(
                                'virtual_account_no', function ($trans) {
...
//                })
                            ...
                            if ($request->get('is_assign') != '') {
                                if ($request->has('is_assign')) {
                                    $query->where(function ($query) use ($request) {
                                        ...
                                        $query->where('role_user.role_id', 'like', "%$by_status%");
                                    });
                                }
                            }
                        })
    ...
    public function getFiListsList(Request $request, $data) {
        ...
        return DataTables::of($data)
                        ...
                        ->addColumn(
                                'address_type', function ($data) use ($type) {
                        ...
                        ->addColumn(
                                'action', function ($data) use ($request) {
    ...
    function getPromgramList($request, $program) {
        return DataTables::of($program)
                        ...
                        ->editColumn(
                                'prgm_id', function ($program) {
                            return $program->prgm_id;
                        })
                        ->editColumn(
                                'f_name', function ($program) {
                            return $program->f_name;
                        })
                        ->editColumn(
                                'product_id', function ($program) {
                            return $program->product_name;
                        })
                        ->editColumn(
                                'prgm_name', function ($program) {
                            return $program->prgm_name;
                        })
                        ->editColumn(
                                'anchor_limit', function ($program) {
                            return \Helpers::formatCurreny($program->anchor_limit);
                        })
                        ->addColumn(
                                'anchor_sub_limit', function ($program) {
                            return '-';
                        })
                        ->addColumn(
                                'loan_size', function ($program) {
                            return '-';
                        })
                        ->editColumn(
                                'status', function ($program) {
    ...
    public function getDocumentsList(Request $request, $documents) {
        ...
        return DataTables::of($documents)
                        ->rawColumns(['is_active'])
                        ->addColumn(
                                'doc_type_id', function ($documents) {
                            return $this->doc_type_ids[$documents->doc_type_id] ?? 'N/A';
                        })
                        ->addColumn(
                                'doc_name', function ($documents) {
                            return $documents->doc_name;
                        })
                        ->addColumn(
                                'product_type', function ($documents) {
                            $productTypes = '';
    ...
    function getSubProgramList($request, $program) {
        return DataTables::of($program)
                        ...
                        ->editColumn(
                                'product_name', function ($program) {
                            return $program->product_name;
                        })
                        ->editColumn(
                                'prgm_type', function ($program) {
    ...
    public function lmsGetCustomers(Request $request, $customer) {
        return DataTables::of($customer)
                        ...
                        ->addColumn(
                                'virtual_acc_id', function ($customer) {
                            return $customer->virtual_acc_id;
                        }
                        )
                        ->editColumn(
                                'customer_name', function ($customer) {
                            $full_name = $customer->user->f_name . ' ' . $customer->user->l_name;
                            return $full_name;
                        }
                        )
                        ->editColumn(
                                'customer_email', function ($customer) {
                            $email = $customer->user->email;
                            return $email;
                        })
                        ->editColumn(
                                'customer_mobile', function ($customer) {
                            $mobile_no = $customer->user->mobile_no;
                            return $mobile_no;
                        })
                        ->editColumn(
                                'limit', function ($customer) {
                            return 12;
                        })
                        ->editColumn(
                                'interest_rate', function ($customer) {
                            return 12;
                        })
                        ->editColumn(
                                'consume_limit', function ($customer) {
                            return 12;
                        })
                        ->editColumn(
                                'available_limit', function ($customer) {

                            return 12;
                        })
                        ->editColumn(
                                'tenor_days', function ($customer) {
                            return 12;
                        })
                        ->editColumn(
                                'assignee', function ($customer) {
                            return 'xyz';
                        })
                        ->editColumn(
                                'assigned_by', function ($customer) {
                            return 'xyz';
                        })
                        ->editColumn(
                            ...
                            if ($request->get('is_assign') != '') {
                                if ($request->has('is_assign')) {
                                    $query->whereHas('user', function($query) use ($request) {
                                        ...
                                        $query->where('is_assigned', 'like', "%$by_status%");
                                    });
                                }
                            }
                        })
    ...
    function getDoaLevelsList($request, $doa) {
        return DataTables::of($doa)
                        ...
                        ->editColumn(
                                'level_code', function ($doa) {
                            return $doa->level_code;
                        })
                        ->editColumn(
                                'level_name', function ($doa) {
    ...
    function getBankAccountList($request, $bank) {
        return DataTables::of($bank)
                        ...
                        ->editColumn(
                                'bank_name', function ($bank) {
                            return $bank->bank_name;
                        })
                        ->addColumn(
                                'action', function ($bank) {
    ...
    function getColenderList($request, $data) {
        return DataTables::of($data)
                        ...
                        ->editColumn(
                                'co_lender_id', function ($data) {
                            return $data->co_lender_id;
                        })
                        ->editColumn(
                                'f_name', function ($data) {
                            return $data->f_name;
                        })
                        ->editColumn(
                                'biz_name', function ($data) {
    ...
    public function getAllGST(Request $request, $data) {
        return DataTables::of($data)
                        ...
                        ->addColumn(
                                'tax_id', function ($data) {
                            return $data->tax_id;
                        })
                        ->addColumn(
                                'tax_val', function ($data) {
