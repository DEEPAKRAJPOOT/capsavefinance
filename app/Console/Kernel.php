...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        if(config('lms.LMS_STATUS') && !\Helpers::checkEodProcess() && \Helpers::getInterestAccrualCronStatus() && !\Helpers::getEodProcessCronStatus()){
            $schedule->command('lms:eodprocess')->timezone(config('common.timezone'))->dailyAt('22:30')->emailOutputOnFailure(config('lms.EOD_FAILURE_MAIL'));
        }

        if(config('lms.LMS_STATUS')){
            $schedule->command('lms:interestaccrual')->timezone(config('common.timezone'))->dailyAt('22:45');
