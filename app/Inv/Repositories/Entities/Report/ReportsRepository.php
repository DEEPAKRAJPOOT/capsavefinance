...
class ReportsRepository extends BaseRepositories implements ReportInterface {
	...
	public function getReconReportData($userId){
		...
		$nonFactorOutstanding = DB::select('SELECT b.customer_id, SUM(COALESCE(a.settled_outstanding,0))AS outstandingAmount FROM rta_transactions AS a JOIN (SELECT * FROM rta_lms_users GROUP BY user_id) AS b ON a.user_id = b.user_id WHERE a.trans_type = \'35\' AND a.entry_type = \'1\' GROUP BY a.`user_id`');
