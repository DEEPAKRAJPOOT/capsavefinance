...
class FinancialJournalItems extends BaseModel {
    ...
    public static function getTallyTxns(array $where = array()) {
      $query = "SELECT tally_entry_id, batch_no, IF(is_debit_credit = 0 , 'Debit', 'Credit') entry_type, trans_type, (CASE WHEN tally_trans_type_id = 1 THEN 'Payment' WHEN tally_trans_type_id = 2 THEN 'Receipt' ELSE 'Journal' END) voucher_type, tally_voucher_code voucher_code, tally_voucher_date voucher_date, invoice_no, invoice_date, ledger_name, amount, ref_no, ref_amount, acc_no, ifsc_code, bank_name, cheque_amount, cross_using, (CASE WHEN mode_of_pay = 1 THEN 'e-fund-Transfer' WHEN mode_of_pay = 2 THEN 'Cheque' WHEN mode_of_pay = 3 THEN 'Nach' ELSE 'Cash' END) as mode_of_pay, inst_no, inst_date, favoring_name, remarks, narration, is_updated is_posted FROM rta_tally_entry ";
