  ...
  public function interestTemp() { 
	...
	public static function getInvoiceDisbursed($disbursalIds){
		return self::whereIn('disbursal_id', $disbursalIds)
				->with('invoice.program_offer.program', 'disbursal')->get();
	}
	  ...
	  function isRepayment()
	{
	 ...
	 public static function getReportAllInvoice()
	 {
		 ...
		 return self::where(['status_id' => 12])->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) >= '".$currentDate."'")/*->with(['invoice','Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])*/->orderBy('invoice_id', 'DESC');
	   ...
	   public static function pdfInvoiceDue($attr)
	 {
		   ...
		   {  
			   ...
			   return self::where('customer_id',$attr->customer_id)->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) >= '".$currentDate."'")->WhereBetween('payment_due_date', [$from_date, $to_date])->where(['status_id' => 12])->with(['Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])->orderBy('invoice_id', 'DESC')->get();
		   ...
		   else if($attr->from_date!=null && $attr->to_date!=null && count($user)==0)
		   {
			   $from_date = Carbon::createFromFormat('d/m/Y', $attr->from_date)->format('Y-m-d');
			   $to_date = Carbon::createFromFormat('d/m/Y', $attr->to_date)->format('Y-m-d'); 
			   return self::WhereBetween('payment_due_date', [$from_date, $to_date])->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) >= '".$currentDate."'")->where(['status_id' => 12])->with(['Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])->orderBy('invoice_id', 'DESC')->get();
		 
		   }
		   else if($attr->from_date==null && $attr->to_date==null && count($user) > 0)
		   {
			  
			  return self::where('customer_id',$attr->customer_id)->where(['status_id' => 12])->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) >= '".$currentDate."'")->with(['Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])->orderBy('invoice_id', 'DESC')->get();
		 
		   }
		else {
		   
			 return self::where(['status_id' => 12])->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) >= '".$currentDate."'")->with(['Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])->orderBy('invoice_id', 'DESC')->get();
	  ...
	  public static function pdfInvoiceOverDue($attr)
	 {
		   ...
		   {  
			   $from_date = Carbon::createFromFormat('d/m/Y', $attr->from_date)->format('Y-m-d');
			   $to_date = Carbon::createFromFormat('d/m/Y', $attr->to_date)->format('Y-m-d'); 
			   return self::where('customer_id',$attr->customer_id)->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) < '".$currentDate."'")->WhereBetween('payment_due_date', [$from_date, $to_date])->where(['status_id' => 12])->with(['InterestAccrual','Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])->orderBy('invoice_id', 'DESC')->get();
		   ...
		   else if($attr->from_date!=null && $attr->to_date!=null && count($user)==0)
		   {
			   $from_date = Carbon::createFromFormat('d/m/Y', $attr->from_date)->format('Y-m-d');
			   $to_date = Carbon::createFromFormat('d/m/Y', $attr->to_date)->format('Y-m-d'); 
			   return self::WhereBetween('payment_due_date', [$from_date, $to_date])->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) < '".$currentDate."'")->where(['status_id' => 12])->with(['InterestAccrual','Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])->orderBy('invoice_id', 'DESC')->get();
		 
		   }
		   else if($attr->from_date==null && $attr->to_date==null && count($user) > 0)
		   {
			  
			  return self::where('customer_id',$attr->customer_id)->where(['status_id' => 12])->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) < '".$currentDate."'")->with(['InterestAccrual','Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])->orderBy('invoice_id', 'DESC')->get();
		 
		   }
		else {
		   
			 return self::where(['status_id' => 12])->whereRaw("ADDDATE(DATE(payment_due_date),grace_period) < '".$currentDate."'")->with(['InterestAccrual','Invoice.business','Invoice.anchor','Invoice.supplier','Invoice.userFile','Invoice.program','Invoice.program_offer','Invoice.Invoiceuser','disbursal.disbursal_batch','Invoice.lms_user'])->orderBy('invoice_id', 'DESC')->get();
