...
class OutstandingReportManual implements ShouldQueue
{
    ...
    private function downloadOutstandingReport($exceldata)
    {
        ...
        ->setCellValue('AD'.$rows, 'Grace Days - Principal')
        ->setCellValue('AE'.$rows, 'Principal Overdue')
        ->setCellValue('AF'.$rows, 'Principal Overdue Category')
        ->setCellValue('AG'.$rows, 'Principal DPD')
        ...
        $filePath = $storage_path.'/Invoice Outstanding Report'.'_'.Carbon::now()->timestamp.'.xlsx';
