...
public function exportCsv($data=[],$columns=[],$fileName='',$extraDataArray=[])
{
  ...
  try {  
    ...
    public function factFileExcel($toExportData, $file_name = "",$moreDetails = []){
      $lastkey = array_key_last($toExportData);
      $objPHPExcel = new PHPExcel();
      ...
      foreach ($toExportData as $title => $data) {
        ...
        // dd($excel_row %2);
          ...
          $objPHPExcel->getActiveSheet()->getStyle('A'. $key .':' . $char . $key)->applyFromArray($styleArray);
          $column = 0;
          $key++;
          $end_row = $key+1;
            $row = $header_row;
            ...
            $objPHPExcel->getActiveSheet()->getStyle('A'. $start_row .':' . $char . $end_row)->applyFromArray($styleArray);
            // $objPHPExcel->getActiveSheet()->getStyle('A'.$start_row .':' . $char . $end_row)->getFill()->getStartColor()->setRGB('FF0000');
            $objPHPExcel->getActiveSheet()->setTitle($sheetTitle);
